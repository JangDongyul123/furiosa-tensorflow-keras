...
# Conv2D 는 (행, 열, 채널) 4차원 입력이 필요하다 -> 데이터를 5 x 2 x 1 로 바꾼다
...
#2. 모델 구성
model = Sequential()
model.add(Conv2D(64, (2, 2), padding='same', activation='relu', input_shape=(5, 2, 1)))
...
mcp = ModelCheckpoint(
    ...
    filepath=path + 'keras34_diabetes.keras',
    verbose=1,
)

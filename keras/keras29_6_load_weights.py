...
#model.save_weights(path + 'keras29_5_save_save_1.weights.h5')

# model = load_model(path + 'keras29_1_save_model.weights')
# 저장된 모델을 가져온다.

#model.load_weights(path+'keras29_5_save_1.weights.h5') #초기 가중치
# weights만 저장하는데, 
# 그러면 모델구성을 저장할 때와 불러올 때를 동일하게 구성해야한다.
# 저장할 때도 불러올 때도 이 코드를 동일하게 유지해야 한다는 뜻이다.
# model = Sequential()
# model.add(Dense(16, input_dim=8 , activation= 'relu'))
# model.add(Dense(8, activation= 'relu'))
# model.add(Dense(4, activation= 'relu'))
# model.add(Dense(2, activation= 'relu'))
# model.add(Dense(1))
# model.compile(loss='mse', optimizer='adam') #컬파일 코드까지 유지

...
x = datasets.data
y = datasets.target
y = y - 1 # sparse_categorical_crossentropy 를 위해 0부터 시작하도록 조정
...
# Conv2D 는 (행, 열, 채널) 4차원 입력이 필요하다 -> 데이터를 9 x 6 x 1 로 바꾼다
x_train = x_train.reshape(-1, 9, 6, 1)
x_val = x_val.reshape(-1, 9, 6, 1)
x_test = x_test.reshape(-1, 9, 6, 1)

#2. 모델 구성
model = Sequential()
model.add(Conv2D(64, (2, 2), padding='same', activation='relu', input_shape=(9, 6, 1)))
...
model.add(Dropout(0.2))
model.add(Dense(7, activation='softmax')) # 다중 분류 -> 클래스 개수만큼 출력, 활성화 함수 softmax
...
mcp = ModelCheckpoint(
    ...
    filepath=path + 'keras34_covtype.keras',
    verbose=1,
)

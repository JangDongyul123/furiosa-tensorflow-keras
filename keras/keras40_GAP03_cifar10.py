...
from tensorflow.keras.layers import Conv2D, Dense, Dropout, Flatten, MaxPooling2D, AveragePooling2D, GlobalAveragePooling2D
...
def create_model_3():
    model = Sequential([
        Conv2D(64, kernel_size=(5, 5), padding='same', activation='relu', input_shape=(32, 32, 3)),
...
def train_or_load(model, x, y, model_path, model_num):
    if os.path.exists(model_path):
        print(f"--- Loading Base Model {model_num} ({model_path}) ---")
        return load_model(model_path)
    else:
        print(f"--- Training Base Model {model_num} ---")
        model.compile(loss='mse', optimizer='adam', metrics=['accuracy'])
        model.fit(x, y, epochs=EPOCHS_SMALL, batch_size=BATCH_SMALL, verbose=1, callbacks=[es])
        model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
        model.fit(x, y, epochs=EPOCHS_LARGE, batch_size=BATCH_LARGE, verbose=1, callbacks=[es])
        model.save(model_path)
        return model

model1 = train_or_load(model1, x_train_base, y_train_base, './_data/CNN/keras36_model1.h5', 1)
model2 = train_or_load(model2, x_train_base, y_train_base, './_data/CNN/keras36_model2.h5', 2)
model3 = train_or_load(model3, x_train_base, y_train_base, './_data/CNN/keras36_model3.h5', 3)
model4 = train_or_load(model4, x_train_base, y_train_base, './_data/CNN/keras36_model4.h5', 4)
model5 = train_or_load(model5, x_train_base, y_train_base, './_data/CNN/keras36_model5.h5', 5)
